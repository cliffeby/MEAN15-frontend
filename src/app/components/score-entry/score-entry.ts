...
export class ScoreEntryComponent implements OnInit, OnDestroy {
  ...
  saveScores(): void {
    ...
    if (this.canSave()) {
      ...
      this.snackBar.open('Cannot save scores. Please check all fields.', 'Close', {
        duration: 3000,
      });
    }
  }
      ...
      if (typeof this.match._id === 'string') {
        this.matchService.updateMatchStatus(this.match._id, 'needs_review').subscribe({
          ...
          error: (err) => {
            ...
            this.snackBar.open('Failed to update match status', 'Close', {
              duration: 4000,
    ...
    this.matchService
      ...
      .subscribe({
        next: (response: any) => {
          ...
          console.log('Match scorecardId._id value:', match.scorecardId._id);
          ...
          if (typeof this.isMatchCompleted !== 'boolean') {
            console.warn(
              '⚠️ isMatchCompleted was not boolean, forcing to false:',
          ...
          console.log(
            'Final isMatchCompleted value:',
            this.isMatchCompleted,
            '(type:',
            typeof this.isMatchCompleted,
            ')',
          );
          ...
          const scorecardId = match.scorecardId._id;
  ...
  private loadScorecardAndMembers(match: Match): void {
    ...
    const matchScorecardId =
      match.scorecardId && typeof match.scorecardId === 'object' && '_id' in match.scorecardId
        ? (match.scorecardId as { _id: string })._id
        : undefined;
    console.log('Loading scorecard and members for match:', match.scorecardId, matchScorecardId);
    // 1. Load the match scorecard (for course)
    const scorecardObservable1 = this.scorecardService.getById(
      matchScorecardId && match.scorecardId && typeof match.scorecardId === 'object'
        ? (match.scorecardId as { id?: string; _id?: string }).id ||
            (match.scorecardId as { _id?: string })._id ||
            ''
        : typeof match.scorecardId === 'string'
          ? match.scorecardId
          : '',
    );

    // 2. Load all members in the match
    const membersObservable1 = match.lineUps
    ...
    if (typeof match.scorecardId === 'string') {
      const matchScorecard = getMatchScorecard(match);
      if (!matchScorecard) {
        console.error('getMatchScorecard returned null for match:', match);
        this.snackBar.open('Invalid scorecard data', 'Close', { duration: 3000 });
        this.loading = false;
        return;
      }
      scorecardId = matchScorecard._id;
      scorecardData = matchScorecard;
  ...
  private setupPlayerScores(members: any[]): void {
    console.log('Setting up player scores for members:', members);
  ...
  private createPlayerFormGroup(): FormGroup {
    const holesArray = this.fb.array(
      Array.from({ length: 18 }, () =>
        this.fb.control(null, [Validators.min(1), Validators.max(15)]),
      ),
    );
      ...
      .subscribe({
        next: (response: ScoresApiResponse) => {
          ...
          scores.forEach((score: any, scoreIndex: number) => {
            ...
            console.log(
              `Looking for member ID: ${memberIdToMatch}, found at index: ${playerIndex}`,
            );
  ...
  onScoreChange(playerIndex: number, holeIndex: number, event: any): void {
    ...
    if (this.isMatchCompleted) {
      ...
      this.snackBar.open(
        'Cannot modify scores - match is completed. Change match status to enable editing.',
        'Close',
        {
          duration: 5000,
          panelClass: ['warning-snackbar'],
  ...
  private async autoSavePlayer(playerIndex: number): Promise<void> {
    ...
    try {
      ...
      this.playerSaveStatus.set(playerIndex, 'error');
      this.snackBar.open(
        `Failed to save scores for ${playerScore.member.firstName}. Will retry on manual save.`,
        'Close',
        {
          duration: 5000,
